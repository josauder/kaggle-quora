...
class Evaluator(object):
	def __init__(self, word_embeddings, first_n_questions = 404246):
		...
		def ident(x):
			...
			self.count+=1
			return x
		self.weights = {word: ident(get_weight(count)) for word, count in counts.items()}
		self.avg_weight/=self.count
		print "Counted words, weighter set up"
		
		self.i=0
		
	def get_word_weight(self,w):
		if self.weights.has_key(w):
			return self.weights[w]
		else:
			return self.avg_weight
			
	def weighted(self,q):
		x = [self.we.embeddings[self.we.words[w]]*self.get_word_weight(w) for w in q if self.we.has_word(w)]
		if len(x)!=0:
			return sum(x)/len(x)
		print "emtpy!"
		return np.zeros(300)
	...
	def process(self, q1, q2, is_duplicate):
		return np.concatenate(( self.weighted(q1),self.weighted(q2), self.unweighted(q1), self.unweighted(q2)))
